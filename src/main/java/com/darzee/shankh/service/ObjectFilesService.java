...
public class ObjectFilesService {
    ...
    public void saveObjectFiles(List<String> FileReferences, String entityType, Long entityId) {
        List<ObjectImagesDAO> objectFilesDAOList = FileReferences
                .stream()
                ...
                .collect(Collectors.toList());
        repo.saveAll(CommonUtils.mapList(objectFilesDAOList, mapper::objectImageDAOToObjectImage));
    }

    public void saveObjectFiles(Map<String, Long> refEntityIdMap, String entityType) {
        List<ObjectImagesDAO> objectFiles = refEntityIdMap.entrySet().stream()
                .map(refEntityIdValue -> new ObjectImagesDAO(refEntityIdValue.getKey(),
                        entityType, refEntityIdValue.getValue()))
                .collect(Collectors.toList());
        repo.saveAll(CommonUtils.mapList(objectFiles, mapper::objectImageDAOToObjectImage));
    }
}
