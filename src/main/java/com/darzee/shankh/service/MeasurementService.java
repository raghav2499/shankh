...
public class MeasurementService {
    ...
    public OverallMeasurementDetails getMeasurementDetails(Long customerId, Long measurementRevisionId,
        ...
        MeasurementRevisions measurementRevision = getMeasurementObject(measurementRevisionId, customerId, orderItemId,
                outfitType);
    ...
    public ResponseEntity saveMeasurementDetails(MeasurementDetailsRequest measurementDetails) throws Exception {
        ...
        CreateMeasurementResponse response = generateCreateMeasurementResponse(measurementsDAO,
                measurementDetails.getCustomerId());
    ...
    public MeasurementsDAO setMeasurementDetails(MeasurementDetailsRequest measurementRequest) throws Exception {
        ...
        if (optionalCustomer.isPresent()) {
            ...
            MeasurementsDAO measurementsDAO = customerMeasurementService.getCustomerMeasurements(customerDAO.getId(),
                    outfitType);
            MeasurementRevisionsDAO revision = null;
            revision = addMeasurementRevision(measurementRequest, customerDAO.getId(), outfitType,
                    measurementRequest.getScale());
            ...
            measurementsDAO = mapper.measurementsToMeasurementDAO(
                    measurementsRepo.save(
                            mapper.measurementsDAOToMeasurement(measurementsDAO, new CycleAvoidingMappingContext())),
                    new CycleAvoidingMappingContext());
    ...
    public ResponseEntity<GetMeasurementRevisionsResponse> getMeasurementRevisions(Long customerId,
        ...
        for (MeasurementRevisionsDAO revision : measurementRevisions) {
            ...
            if (!CollectionUtils.isEmpty(revision.getMeasurementValue())) {
                ...
                MeasurementRevisionImageDetail measurementRevisionImageDetail = new MeasurementRevisionImageDetail(
                        referenceId, measurementRevImageUrl);
    ...
    public List<InnerMeasurementDetails> generateInnerMeasurementDetails(Long boutiqueId,
        ...
        List<MeasurementParamDAO> measurementParamDetails = mapper
                .measurementParamToDAOList(measurementParamRepo.findAllByNameIn(eligibleMeasurementParams));
        ...
        if (boutiqueMeasurementParams.keySet().contains(OutfitSide.TOP)) {
            ...
            List<MeasurementDetails> measurementDetails = getMeasurementDetailsList(revisionsDAO.getMeasurementValue(),
                    CM_TO_INCH_DIVIDING_FACTOR,
                    paramList, paramDetailMap, nonEmptyValuesOnly);
            // String heading
            // =localisationService.translate(outfitTypeService.getTopHeading());
        ...
        innerMeasurementDetailsList = measurementDetailsTranslator
                .translateInnerMeasurementDetailsList(innerMeasurementDetailsList);
    ...
    private void validateGetMeasurementRequestParams(Long customerId, Long measurementRevisionId,
        ...
        if (outfitTypeIndex != null && !OutfitType.getOutfitOrdinalEnumMap().containsKey(outfitTypeIndex)) {
            String errorMessage = errorMessageTranslator
                    .getTranslatedMessage(ErrorMessages.OUTFIT_TYPE_NOT_SUPPORTED_ERROR);
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, errorMessage);
        }
        if (scale != null && !MeasurementScale.getEnumMap().containsKey(scale)) {
            String errorMessage = errorMessageTranslator
                    .getTranslatedMessage(ErrorMessages.INVALID_MEASUREMENT_SCALE_ERROR);

            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, errorMessage);
        }
    }

    private CreateMeasurementResponse generateCreateMeasurementResponse(MeasurementsDAO measurementDAO,
            Long customerId) {
        String successMessage = successMessageTranslator
                .getTranslatedMessage(SuccesssMessages.MEASUREMENT_DETAILS_SAVE_SUCCESS);
        ...
        if (!CollectionUtils.isEmpty(customerMeasurementDetails)) {
            for (String boutiqueParam : boutiqueParams) {
                if (customerMeasurementDetails.containsKey(boutiqueParam)) {
                    measurementValue.put(boutiqueParam,
                            customerMeasurementDetails.get(boutiqueParam) * multiplyingFactor);
                }
            }
        }
        MeasurementRevisionsDAO revisions = new MeasurementRevisionsDAO(customerId, outfitType, measurementValue);
    ...
    private OverallMeasurementDetails setMeasurementDetails(MeasurementRevisionsDAO revisionsDAO,
        ...
        if (CollectionUtils.isEmpty(revisionsDAO.getMeasurementValue())
            ...
            measurementRevisionImgLink = measurementRevisionService
                    .getMeasurementRevisionImageLink(revisionsDAO.getId());
    ...
    private Map<OutfitSide, List<String>> getBoutiqueMeasurementParams(Long boutiqueId, OutfitType outfitType) {
        ...
        if (outfitType.getPieces() == 2) {
            BoutiqueMeasurement topOutfitMeasurement = boutiqueMeasurementRepo
                    .findByBoutiqueIdAndOutfitTypeAndOutfitSide(boutiqueId, outfitType, OutfitSide.TOP);
            if (topOutfitMeasurement == null) {
                topOutfitMeasurement = boutiqueMeasurementRepo.findByBoutiqueIdAndOutfitTypeAndOutfitSide(
                        defaultBoutiqueId,
                        outfitType, OutfitSide.TOP);
            }
            BoutiqueMeasurement bottomOutfitMeasurement = boutiqueMeasurementRepo
                    .findByBoutiqueIdAndOutfitTypeAndOutfitSide(boutiqueId, outfitType, OutfitSide.BOTTOM);
            if (bottomOutfitMeasurement == null) {
                bottomOutfitMeasurement = boutiqueMeasurementRepo.findByBoutiqueIdAndOutfitTypeAndOutfitSide(
                        defaultBoutiqueId,
            ...
            BoutiqueMeasurement outfitMeasurement = boutiqueMeasurementRepo.findByBoutiqueIdAndOutfitType(boutiqueId,
                    outfitType);
            if (outfitMeasurement == null) {
                outfitMeasurement = boutiqueMeasurementRepo.findByBoutiqueIdAndOutfitType(defaultBoutiqueId,
                        outfitType);
            }
