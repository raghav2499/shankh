...
public class AmazonClient {
    ...
    private String generateShortLivedUrl(String bucketName, String fileName) {
        ...
        String longUrl = "https://s3." + region + ".amazonaws.com/" + bucketName + "/" + fileName;
    
        try {
            URL url = new URL("http://tinyurl.com/api-create.php?url=" + URLEncoder.encode(longUrl, "UTF-8"));
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            BufferedReader rd = new BufferedReader(new InputStreamReader(conn.getInputStream()));
            String shortUrl = rd.readLine();
            return shortUrl;
        } catch (Exception e) {
